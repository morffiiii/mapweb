...
class TrackingService: Service(), LocationListener {
    ...
    private fun accept(p: Point) {
        ...
        val next=p.copy(gap=s.breakNext)
    ...
    override fun onLocationChanged(l: Location) {
        ...
        val p=Point(l.latitude,l.longitude,l.time,l.accuracy.toDouble()); store.position=p; accept(p); store.notifyChanged()
    }
    private fun pause() { store.pending=null; store.active?.let { if(it.pausedAt==null) it.pausedAt=System.currentTimeMillis() }; store.save(); store.message="Запись на паузе"; store.notifyChanged(); stopSelf() }
